...
-- The first time the assignee calls the API to select the document, OpenedAt gets set
	...
	CONSTRAINT [FK_DocumentAssignments__AssigneeId] FOREIGN KEY ([AssigneeId]) REFERENCES [dbo].[LocalUsers] ([Id]),
	CONSTRAINT [FK_DocumentAssignments__CreatedById] FOREIGN KEY ([CreatedById]) REFERENCES [dbo].[LocalUsers] ([Id])
);
GO
